...
def run_models(method, error_rate, X_train, y_train, X_test, y_test):
    if method == 'LinearSVC':
        error_rate.append(linear_svc_error_rate(X_train, y_train, X_test, y_test))

    if method == 'SVC':
        error_rate.append(svc_error_rate(X_train, y_train, X_test, y_test))

    if method == 'LogisticRegression':
        error_rate.append(logistic_regression_error_rate(X_train, y_train, X_test, y_test))

    if method == 'MultiGaussClassify':
        error_rate.append(multi_gauss_classify_error_rate(X_train, y_train, X_test, y_test))
...
def multi_gauss_classify_error_rate(X_train, y_train, X_test, y_test):
    mgc = MultiGaussClassify()
    mgc.fit(X_train, y_train)
    y_test_pred = mgc.predict(X_test)
    return get_error_rate(y_test, y_test_pred)
