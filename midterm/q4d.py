...
def forward_calc(transition_matrix, new_transition_matrix, evidence_matrix, init_prob, observations):
    forward_count = 3
    filtering_res = filtering_path(transition_matrix, evidence_matrix, init_prob, forward_count, observations)
    return filtering_res


def smoothing(transition_matrix, evidence_matrix, observations):
    res = np.ones((2, 1))
    for i in range(0, 3):
...
def init():
    ...
    forward = forward_calc(transition_matrix, new_transition_matrix, evidence_matrix, init_prob, observations)
    backward = smoothing(transition_matrix, evidence_matrix, observations)
    out = np.multiply(forward, backward)
    out[:] = out[:]/np.sum(out[:])
    print out
